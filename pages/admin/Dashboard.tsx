...
import { ShoppingCart, Users, CreditCard, UserCheck, User, TrendingUp, Calendar, Bell, ArrowRight, Activity, DollarSign } from 'lucide-react';
  ...
  const StatsCard = ({ title, value, subtext, icon: Icon, colorClass, bgClass }: any) => (
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
          <div className="flex justify-between items-start mb-4">
              ...
              <span className={`text-xs font-bold px-2 py-1 rounded-lg ${bgClass} ${colorClass} flex items-center`}>
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 bg-white p-1 rounded-xl border border-gray-200 shadow-sm">
             <button className="px-4 py-2 bg-gray-900 text-white rounded-lg text-xs font-bold shadow-md">Last 7 Days</button>
             <button className="px-4 py-2 text-gray-500 hover:bg-gray-50 rounded-lg text-xs font-bold">Last Month</button>
             <button className="px-4 py-2 text-gray-500 hover:bg-gray-50 rounded-lg text-xs font-bold">Last Year</button>
          </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 h-auto">
         ...
         <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex flex-col h-[400px]">
            <div className="flex justify-between items-center mb-6">
               ...
               <button className="text-gray-400 hover:text-gray-600"><TrendingUp className="w-5 h-5" /></button>
            </div>
                ...
                {[1,2,3,4,5].map((_, i) => (
                    <div key={i} className="flex gap-4 items-start group">
                        ...
                        <div className="pb-2">
                           ...
                           <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wide">2 mins ago</span>
                        </div>
                    </div>
                ))}
                 <div className="flex gap-4 items-start group">
                    ...
                    <div className="pb-2">
                        ...
                        <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wide">1 hour ago</span>
                    </div>
                </div>
             </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="px-6 py-5 border-b border-gray-100 flex justify-between items-center">
          ...
          <button className="text-primary-600 text-xs font-bold hover:underline flex items-center">View All <ArrowRight className="w-3 h-3 ml-1" /></button>
        </div>
            ...
            <tbody className="divide-y divide-gray-50">
              {bookings.slice(0, 5).map((booking) => (
                <tr key={booking.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-right font-bold text-gray-900">${booking.totalPrice}</td>
                </tr>
              ))}
            </tbody>
          </table>
