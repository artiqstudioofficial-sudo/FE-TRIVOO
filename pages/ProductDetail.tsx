...
import { MapPin, Star, Check, Calendar, CreditCard, User, Share2, Heart, ArrowLeft, Clock, Utensils, BedDouble, Info, Car, Navigation, Image, ArrowRight, Maximize2, X, ChevronLeft, ChevronRight, Mountain, Coffee, Home, Mail, Phone, Zap } from 'lucide-react';
import { mockService } from '../services/mockService';
import { useAuth } from '../AuthContext';
import { useToast } from '../components/ToastContext';
import { useWishlist } from '../components/WishlistContext';
import { Product, TourDetails, StayDetails, CarDetails, TransportCategory } from '../types';

const ProductDetail: React.FC = () => {
  // ... (component logic remains the same until the return statement) ...
  ...
  useEffect(() => {
    if (id) {
      ...
      mockService.getProductById(Number(id)).then(p => {
        setProduct(p);
        if (p) {
          mockService.getRelatedProducts(p.categoryId, p.id).then(setRelatedProducts);
        }
      });
      // Reset states
      setCheckIn('');
  ...
  const isSingleDaySelection = isTour(details) || (isCar(details) && details.transportCategory === TransportCategory.AIRPORT_TRANSFER);

  // --- FLASH SALE LOGIC ---
  const activeFlashSale = product.flashSale && product.flashSale.status === 'approved' ? product.flashSale : null;
  ...
  const isDateBlocked = (dateStr: string) => {
      if (!product.blockedDates) return false;
      return product.blockedDates.includes(dateStr);
  };
  ...
  const handleDateSelect = (day: number) => {
      ...
      // 2. Single Day Logic (Tour/Transfer)
      ...
      if (calendarMode === 'checkIn') {
          ...
          setCalendarMode('checkOut'); // Auto switch to checkout
      } else {
          // CheckOut mode
          if (parseDateLocal(dateStr) <= parseDateLocal(checkIn)) {
              // If clicking before or on CheckIn, treat as new CheckIn
              if (isBlocked) {
                  showToast('Check-in date is unavailable.', 'error');
                  return;
              }
          ...
          // Validate range: Ensure NO night between checkIn and checkOut is blocked.
          // Checking out ON a blocked date is ALLOWED (you leave before the night).
          let isValidRange = true;
          let current = parseDateLocal(checkIn);
          const end = parseDateLocal(dateStr);
          
          // Loop dates from CheckIn UP TO (but not including) CheckOut
  ...
  const renderCalendar = () => {
      ...
      for (let day = 1; day <= daysInMonth; day++) {
        ...
        if (!isDisabled) {
            if (isSingleDaySelection && isBlocked) isDisabled = true;
            else if (calendarMode === 'checkIn' && isBlocked) isDisabled = true;
            // For checkOut, blocked dates are enabled (valid for leaving)
        }
        ...
        if (isSingleDaySelection) {
             isSelected = checkIn === dateStr;
        } else {
             isSelected = checkIn === dateStr || checkOut === dateStr;
             if (checkIn && checkOut) {
        ...
        const showBlockedStyle = !isDisabled && isBlocked;

        days.push(
          <button
            key={day}
            onClick={(e) => { e.preventDefault(); !isDisabled && handleDateSelect(day); }}
            disabled={isDisabled}
            ...
            `}
            title={isBlocked ? (calendarMode === 'checkOut' && !isSingleDaySelection ? 'Available for Checkout' : 'Fully Booked') : isPast ? 'Past Date' : 'Available'}
          >
            {day}
          </button>
        );
  ...
  const handleBookNow = (e: React.FormEvent | React.MouseEvent) => {
    ...
    // Smooth Scroll to Calendar if date not selected
    if (!checkIn) {
    ...
    // IMPORTANT: Pass effective price to payment
    const productForPayment = {
        ...product,
        price: effectivePrice // Ensure payment page uses sale price
    };
    ...
    navigate('/payment', {
      state: {
        ...
        date: isSingleDaySelection ? formatDateDisplay(checkIn) : `${formatDateDisplay(checkIn)} - ${formatDateDisplay(checkOut)}`, 
    ...
    <div className="bg-gray-50 min-h-screen pb-24">
      ...
      <div className="h-[40vh] md:h-[60vh] relative group cursor-pointer" onClick={() => openLightbox(0)}>
        ...
        <div className="absolute top-0 w-full p-4 md:p-8 flex justify-between items-start z-10 pt-24 md:pt-28">
           <button onClick={(e) => { e.stopPropagation(); navigate(-1); }} className="bg-white/20 backdrop-blur-md hover:bg-white text-white hover:text-gray-900 p-3 rounded-full transition-all">
           ...
           <div className="flex gap-3">
              ...
              <button onClick={(e) => { e.stopPropagation(); handleShare(); }} className="bg-white/20 backdrop-blur-md hover:bg-white text-white hover:text-blue-500 p-3 rounded-full transition-all">
          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="flex flex-wrap items-center text-white/90 gap-4 md:gap-8 text-sm md:text-base">
              ...
              <div className="flex items-center">
                 ...
                 <span className="ml-1 opacity-70">({product.reviews?.length || 0} reviews)</span>
              </div>
              {isTour(details) && (
                 <div className="flex items-center">
            ...
            <div className="bg-white rounded-3xl p-8 md:p-10 shadow-sm border border-gray-100 mb-8">
               <div className="flex space-x-6 border-b border-gray-100 mb-6 overflow-x-auto no-scrollbar">
                  <button 
                     ...
                     className={`pb-4 text-sm font-bold uppercase tracking-wide whitespace-nowrap ${activeTab === 'overview' ? 'text-primary-600 border-b-2 border-primary-600' : 'text-gray-400 hover:text-gray-600'}`}
                  >
                     Overview
                  </button>
                  {isTour(details) && (
                     <button 
                        ...
                        className={`pb-4 text-sm font-bold uppercase tracking-wide whitespace-nowrap ${activeTab === 'itinerary' ? 'text-primary-600 border-b-2 border-primary-600' : 'text-gray-400 hover:text-gray-600'}`}
                     >
                        Itinerary
                     </button>
                  )}
                  ...
                  <button 
                     ...
                     className={`pb-4 text-sm font-bold uppercase tracking-wide whitespace-nowrap ${activeTab === 'gallery' ? 'text-primary-600 border-b-2 border-primary-600' : 'text-gray-400 hover:text-gray-600'}`}
                  >
                     Gallery
                  </button>
                  ...
                  <button 
                     ...
                     className={`pb-4 text-sm font-bold uppercase tracking-wide whitespace-nowrap ${activeTab === 'reviews' ? 'text-primary-600 border-b-2 border-primary-600' : 'text-gray-400 hover:text-gray-600'}`}
                  >
                     Reviews
                  </button>
               </div>
               ...
               {activeTab === 'overview' && (
                  <div className="animate-in fade-in">
                     ...
                     {details && 'rules' in details && details.rules && (
                        <>
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {galleryImages.map((img, index) => (
                           <div 
                                ...
                                className={`relative rounded-2xl overflow-hidden group shadow-sm cursor-pointer ${index === 0 ? 'md:col-span-2 md:h-80' : 'h-48'}`}
                            >
                           ...
                           <div className="bg-gray-50 rounded-xl p-4 text-sm space-y-3 border border-gray-100">
                                 {day.accommodation && (
                                    <div className="flex items-center text-gray-700">
                                       ...
                                       <div>
                                          <span className="text-xs font-bold text-gray-400 uppercase block">Accommodation</span>
                                 ...
                                 {day.meals && day.meals.length > 0 && (
                                    <div className="flex items-center text-gray-700">
                                       ...
                                       <div>
                                          <span className="text-xs font-bold text-gray-400 uppercase block">Meals Included</span>
            ...
            {relatedProducts.length > 0 && (
               <div className="mt-16 animate-in fade-in slide-in-from-bottom-8">
                  <div className="flex items-center justify-between mb-8">
                     ...
                     <Link to="/explore" className="text-primary-600 font-bold text-sm hover:underline flex items-center">
            ...
            <div className="bg-white rounded-3xl shadow-xl border border-gray-100 p-8 sticky top-28 relative overflow-hidden">
              ...
              <div className="flex justify-between items-end mb-8 pb-6 border-b border-gray-100 mt-4">
                <div>
                  ...
                  <div className="flex items-end gap-2 mt-1">
                      {activeFlashSale && (
                          <span className="text-lg text-gray-400 line-through mb-1">{product.currency} {product.price}</span>
                      )}
              ...
              <form onSubmit={handleBookNow} className="space-y-5">
                ...
                <div className="relative" ref={calendarRef}>
                    ...
                    <div 
                        onClick={() => { setIsCalendarOpen(true); setCalendarMode('checkIn'); }}
                    ...
                    >
                        <div className="flex items-center text-gray-700 text-sm font-medium">
                            ...
                            {checkIn ? (
                                isSingleDaySelection ? formatDateDisplay(checkIn) : `${formatDateDisplay(checkIn)} — ${checkOut ? formatDateDisplay(checkOut) : 'End Date'}`
                            ) : (
                                <span className="text-gray-400">Select {isSingleDaySelection ? 'Date' : 'Dates'}</span>
                            )}
                        </div>
                    </div>
                ...
                <div>
                  ...
                  {!isTour(details) && (
                     <p className="text-[10px] text-gray-400 mt-1.5 ml-1">
                        Max capacity per {unitLabel.toLowerCase()}: {capacity} {itemLabel.toLowerCase()}s
                     </p>
                  )}
                </div>
