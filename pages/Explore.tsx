...
import { MapPin, Star, Filter, X, Calendar, Search, ArrowRight, SlidersHorizontal, ArrowUpDown, Heart, ChevronDown } from 'lucide-react';
import { mockService } from '../services/mockService';
import { useWishlist } from '../components/WishlistContext';
import { Product, Category, TourCategory, StayCategory, TransportCategory } from '../types';
        ...
        <div className="bg-white p-4 md:p-6 rounded-3xl shadow-soft border border-gray-100 mb-10 relative">
          <div className="flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-4">
               ...
               {/* MAIN CATEGORIES */}
               <div>
                   <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2 ml-1">Categories</p>
                   <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar w-full">
                        <button
                        onClick={() => handleCategorySelect(null)}
                        className={`px-5 py-2.5 rounded-full text-sm font-bold whitespace-nowrap transition-all flex-shrink-0 border ${!selectedCategory ? 'bg-gray-900 text-white border-gray-900 shadow-md' : 'bg-white text-gray-600 border-gray-200 hover:bg-gray-50'}`}
                        >
                        All
                        </button>
                        {isLoading ? (
                            [1,2,3].map(i => <div key={i} className="w-24 h-10 bg-gray-200 rounded-full animate-pulse flex-shrink-0"></div>)
                        ) : (
                            categories.map(cat => (
                                <button
                                    key={cat.id}
                                    onClick={() => handleCategorySelect(cat.id)}
                                    className={`px-5 py-2.5 rounded-full text-sm font-bold whitespace-nowrap transition-all flex-shrink-0 border ${selectedCategory === cat.id ? 'bg-gray-900 text-white border-gray-900 shadow-md' : 'bg-white text-gray-600 border-gray-200 hover:bg-gray-50'}`}
                                >
                                    {cat.name}
                                </button>
                            ))
               ...
               {selectedCategory && subCategories.length > 0 && (
                   <div className="animate-in fade-in slide-in-from-top-2">
                       <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2 ml-1">Type of Experience</p>
                       <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar w-full">
                           <button
                               onClick={() => setSelectedSubCategory(null)}
                               className={`px-4 py-2 rounded-xl text-xs font-bold whitespace-nowrap transition-all flex-shrink-0 border ${!selectedSubCategory ? 'bg-primary-100 text-primary-700 border-primary-200' : 'bg-white text-gray-500 border-gray-100 hover:bg-gray-50'}`}
                           >
                           ...
                           {subCategories.map((sub) => (
                               <button
                                   ...
                                   className={`px-4 py-2 rounded-xl text-xs font-bold whitespace-nowrap transition-all flex-shrink-0 border ${selectedSubCategory === sub ? 'bg-primary-600 text-white border-primary-600 shadow-sm' : 'bg-white text-gray-500 border-gray-100 hover:bg-gray-50'}`}
                               >
                                   {sub}
                               </button>
                           ))}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
          {isLoading ? (
            ...
            filteredProducts.map((product) => {
              ...
              return (
                <Link key={product.id} to={`/product/${product.id}`} className="group bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 border border-gray-100 hover:-translate-y-1 flex flex-col relative">
                
                {/* Unavailable Overlay */}
                {(!product.isActive || (dateQuery && product.blockedDates?.includes(dateQuery))) && (
                ...
                <div className="aspect-[4/3] relative overflow-hidden">
                    ...
                    >
                        <Heart 
                          className={`w-4 h-4 transition-colors ${isSaved ? 'text-red-500 fill-red-500' : 'text-gray-400 group-hover/btn:text-red-500'}`} 
                        />
                    </button>
                ...
                <div className="p-6 flex-1 flex flex-col">
                    <div className="flex justify-between items-start mb-2">
                        ...
                        {subLabel && (
                            <span className="text-[10px] bg-gray-100 text-gray-500 px-2 py-0.5 rounded-md font-bold uppercase">{subLabel}</span>
                        )}
                    </div>
        ...
        {!isLoading && filteredProducts.length === 0 && (
          <div className="text-center py-24 bg-white rounded-3xl shadow-sm border border-gray-100 animate-in fade-in zoom-in duration-300">
            ...
            <p className="text-gray-500 mb-8 max-w-md mx-auto text-lg">
                We couldn't find any {selectedSubCategory || ''} {categories.find(c => c.id === selectedCategory)?.name.toLowerCase() || 'items'} matching your search.
            </p>
            <button onClick={clearSearch} className="px-8 py-3 bg-primary-600 text-white rounded-xl font-bold hover:bg-primary-700 transition-colors shadow-lg shadow-primary-600/20">
               Clear All Filters
            </button>
          </div>
        )}
