...
import { ArrowRight, MapPin, Search, Calendar, Users, ChevronLeft, ChevronRight, Timer, Sparkles, Brain, Map, RefreshCw, Zap, ShieldCheck, Award, Lock, Headphones, BadgePercent, TrendingUp, Car, Plane, Building2, Palmtree, Flame, Quote, Star, Clock } from 'lucide-react';
import { mockService } from '../services/mockService';
import { Category, Product, FlashSaleCampaign } from '../types';
...
const DESTINATION_STORIES = [
  ...
  { name: 'Japan', image: 'https://images.unsplash.com/photo-1493976040374-85c8e12f0c0e?auto=format&fit=crop&w=300&q=80' },
  { name: 'Paris', image: 'https://images.unsplash.com/photo-1502602898657-3e91760cbb34?auto=format&fit=crop&w=300&q=80' },
  { name: 'Swiss', image: 'https://images.unsplash.com/photo-1530122037265-a5f1f91d3b99?auto=format&fit=crop&w=300&q=80' },
  { name: 'Italy', image: 'https://images.unsplash.com/photo-1529260830199-42c42dda5f30?auto=format&fit=crop&w=300&q=80' },
  { name: 'Greece', image: 'https://images.unsplash.com/photo-1533105079780-92b9be482077?auto=format&fit=crop&w=300&q=80' },
  { name: 'New York', image: 'https://images.unsplash.com/photo-1496442226666-8d4a0e62e6e9?auto=format&fit=crop&w=300&q=80' },
  { name: 'London', image: 'https://images.unsplash.com/photo-1513635269975-59663e0ac1ad?auto=format&fit=crop&w=300&q=80' }
];
...
const Home: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const currentCampaign = campaigns.find(c => c.isActive && c.startDate <= today && c.endDate >= today);
  ...
  const scrollFlashSale = (direction: 'left' | 'right') => {
      if (flashSaleRef.current) {
          ...
          const newScrollPosition = flashSaleRef.current.scrollLeft + (direction === 'right' ? scrollAmount : -scrollAmount);
  ...
  const renderCalendarGrid = () => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      days.push(
        <button
          key={day}
          onClick={(e: { preventDefault: () => void; }) => { e.preventDefault(); handleDateClick(day); }}
  ...
  return (
    <div>
      ...
      <div className="relative min-h-[100dvh] md:h-[90vh] flex items-center justify-center px-4 pt-28 md:pt-0 pb-12">
        ...
        <div className="relative z-20 w-full max-w-7xl mx-auto text-center px-4">
          ...
          <h1 className="text-4xl md:text-6xl lg:text-8xl font-serif font-bold text-white mb-6 leading-tight tracking-tight drop-shadow-sm">
            Find Your <br className="md:hidden" /> <span className="text-transparent bg-clip-text bg-gradient-to-r from-teal-200 to-white">{typewriterText}</span>
             ...
             >
                ...
                <div className="flex-1 flex items-center md:border-r md:border-gray-200 p-4 md:p-3 md:pl-8 relative" ref={searchRef}>
                   ...
                   <div className="text-left w-full relative">
                      <label className="block text-[10px] md:text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-0.5">
                         {searchCategory === 'cars' ? 'Pick-up Location' : searchCategory === 'transfers' ? 'From/To' : 'Destination'}
                      </label>
                      <input 
                       type="text" 
                     ...
                     <div className="absolute top-full left-1/2 transform -translate-x-1/2 mt-4 bg-white rounded-2xl shadow-2xl p-6 w-[280px] md:w-[320px] z-[70] animate-in fade-in slide-in-from-top-2 border border-gray-100 ring-1 ring-black/5">
                       ...
                       <div className="grid grid-cols-7 mb-3 text-center">
                         {['S', 'M', 'T', 'W', 'T', 'F', 'S'].map((day, i) => (
                           <span key={i} className="text-[10px] font-bold text-gray-400 uppercase tracking-wide">{day}</span>
                         ))}
                       </div>
                       ...
                       <div className="flex justify-between items-center pt-4 border-t border-gray-100">
                         <button 
                           onClick={(e: { preventDefault: () => void; }) => { e.preventDefault(); handleClear(); }}
                           className="text-xs font-bold uppercase tracking-wide text-gray-400 hover:text-gray-800 transition-colors"
                         >
                           Clear
                         </button>
                         <button 
                           onClick={(e: { preventDefault: () => void; }) => { e.preventDefault(); handleToday(); }}
                ...
                <div className="flex-1 flex items-center p-4 md:p-3 md:px-6">
                   ...
                   <div className="text-left w-full">
                      <label className="block text-[10px] md:text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-0.5">
                         {searchCategory === 'cars' || searchCategory === 'transfers' ? 'Passengers' : 'Guests'}
                      </label>
      ...
      <div className="bg-white py-8 md:py-12 border-b border-gray-100 relative z-0">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
           <div className="flex items-center justify-between mb-6 md:mb-8">
              ...
              <Link to="/explore" className="text-sm font-bold text-primary-600 hover:text-primary-700 transition-colors flex items-center">
           ...
           <div className="flex items-center justify-start md:justify-center gap-4 sm:gap-6 lg:gap-8 overflow-x-auto md:overflow-visible py-4 -mx-4 px-4 md:mx-0 md:px-0 no-scrollbar md:flex-nowrap">
              {DESTINATION_STORIES.map((dest, index) => (
                ...
                >
                  ...
                  <span className="mt-3 text-xs md:text-sm font-bold text-gray-700 group-hover:text-primary-600 transition-colors transform group-hover:translate-y-0.5 block">{dest.name}</span>
                </div>
              ))}
           </div>
        </div>
      ...
      <div className={`py-16 md:py-24 overflow-hidden relative transition-colors duration-500 ${activeCampaign ? 'text-white' : 'bg-gradient-to-r from-orange-50 via-amber-50 to-orange-50'}`}>
        ...
        {activeCampaign ? (
            <div className="absolute inset-0 z-0">
                <img src={activeCampaign.image} className="w-full h-full object-cover" alt="Campaign Background" />
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-6">
             <div className="mb-2 md:mb-0">
               <div className="flex items-center gap-2 mb-3">
                 <div className={`text-[10px] font-extrabold px-3 py-1.5 rounded-full uppercase tracking-wider animate-pulse flex items-center shadow-lg ${activeCampaign ? 'bg-yellow-400 text-black shadow-yellow-400/30' : 'bg-red-500 text-white shadow-red-500/30'}`}>
                   {activeCampaign ? <Sparkles className="w-3.5 h-3.5 mr-1.5" /> : <Flame className="w-3.5 h-3.5 mr-1.5 fill-white" />} 
                   {activeCampaign ? 'SPECIAL EVENT' : 'FLAME HOT'}
                 </div>
                 <div className={`flex items-center text-xs font-bold gap-1 ${activeCampaign ? 'text-yellow-400' : 'text-orange-600'}`}>
                    <TrendingUp className="w-4 h-4" />
                    <span className="uppercase tracking-wide">High Demand</span>
                 </div>
               </div>
               
               <h2 className={`text-3xl md:text-5xl font-serif font-bold leading-tight text-left ${activeCampaign ? 'text-white' : 'text-gray-900'}`}>
                   {activeCampaign ? activeCampaign.name : <>Flash Sale <br className="hidden md:block"/> Ending Soon</>}
               </h2>
               {activeCampaign && (
             ...
             {/* Countdown Timer */}
             <div className={`flex items-center gap-6 px-8 py-5 rounded-2xl shadow-xl justify-between md:justify-start w-full md:w-auto ${activeCampaign ? 'bg-white/10 backdrop-blur-md border border-white/20' : 'bg-white shadow-orange-100/50 border border-orange-100'}`}>
                <div className="flex items-center">
                <div className={`p-2 rounded-lg mr-3 ${activeCampaign ? 'bg-white/20' : 'bg-red-50'}`}>
                    <Timer className={`w-6 h-6 animate-pulse ${activeCampaign ? 'text-white' : 'text-red-500'}`} />
                </div>
                <div className="flex flex-col">
                    <span className={`font-bold text-sm ${activeCampaign ? 'text-white' : 'text-gray-900'}`}>Offer Ends In</span>
                    <span className={`text-[10px] font-bold uppercase tracking-wider ${activeCampaign ? 'text-yellow-400' : 'text-red-500'}`}>Don't Miss Out</span>
                </div>
                </div>
                <div className="flex gap-2 items-center">
                <div className={`rounded-lg p-2 min-w-[40px] text-center ${activeCampaign ? 'bg-black/50 text-white' : 'bg-gray-900 text-white'}`}>
                    <span className="text-xl font-mono font-bold block leading-none">{timeParts.h}</span>
                    <span className="text-[9px] text-gray-400 font-bold uppercase">Hrs</span>
                </div>
                <span className={`font-bold ${activeCampaign ? 'text-white/50' : 'text-gray-300'}`}>:</span>
                <div className={`rounded-lg p-2 min-w-[40px] text-center ${activeCampaign ? 'bg-black/50 text-white' : 'bg-gray-900 text-white'}`}>
                    <span className="text-xl font-mono font-bold block leading-none">{timeParts.m}</span>
                    <span className="text-[9px] text-gray-400 font-bold uppercase">Min</span>
                </div>
                <span className={`font-bold ${activeCampaign ? 'text-white/50' : 'text-gray-300'}`}>:</span>
                <div className={`rounded-lg p-2 min-w-[40px] text-center shadow-lg ${activeCampaign ? 'bg-yellow-500 text-black shadow-yellow-500/30' : 'bg-red-500 text-white shadow-red-500/30'}`}>
                    <span className="text-xl font-mono font-bold block leading-none">{timeParts.s}</span>
                    <span className={`text-[9px] font-bold uppercase ${activeCampaign ? 'text-black/70' : 'text-white/80'}`}>Sec</span>
                </div>
                </div>
             </div>
          </div>
               ...
               >
                  ...
                  <div className="h-64 md:h-72 relative overflow-hidden">
                     ...
                     <div className="absolute top-4 left-4 flex flex-col gap-2 mt-4">
                        <div className={`text-white text-xs font-extrabold px-3 py-1.5 rounded-lg shadow-lg z-10 tracking-wide w-fit ${isCampaignProduct ? 'bg-purple-600' : 'bg-red-600'}`}>
                  ...
                  <div className="p-6 md:p-7 flex-1 flex flex-col justify-between relative bg-white">
                     ...
                     <div className="mb-6">
                        ...
                        <div className="w-full bg-gray-100 rounded-full h-2.5 overflow-hidden">
                           <div 
                              className={`h-full rounded-full transition-all duration-1000 ${isCampaignProduct ? 'bg-gradient-to-r from-purple-500 to-indigo-600' : 'bg-gradient-to-r from-orange-400 to-red-600'}`}
                              style={{ width: `85%` }}
                           ></div>
                        </div>
                     </div>
                     ...
                     <div className="flex items-center justify-between pt-4 border-t border-gray-100">
                        <div>
                           <span className="text-gray-400 line-through text-sm font-medium block mb-0.5">{product.currency} {product.price}</span>
                           <span className="text-2xl font-bold text-red-600 tracking-tight">{product.currency} {product.flashSale?.salePrice}</span>
                        </div>
                        <button className={`text-white px-6 py-3 rounded-xl font-bold text-sm shadow-xl transition-all active:scale-95 flex items-center ${isCampaignProduct ? 'bg-purple-600 shadow-purple-600/20 hover:bg-purple-700' : 'bg-gray-900 shadow-gray-900/10 hover:bg-red-600 hover:shadow-red-600/30'}`}>
          ...
          {flashSaleProducts.length > 0 && (
            <div className="flex justify-center gap-4 mt-8">
                <button 
                  ...
                  className={`p-3 rounded-full shadow-md transition-all hover:scale-110 active:scale-95 group ${activeCampaign ? 'bg-white/20 text-white hover:bg-white/30 backdrop-blur-sm' : 'bg-white border border-gray-100 text-gray-700 hover:bg-gray-50'}`}
                ...
                <button 
                  ...
                  className={`p-3 rounded-full shadow-md transition-all hover:scale-110 active:scale-95 group ${activeCampaign ? 'bg-white text-gray-900 hover:bg-white/90' : 'bg-gray-900 border border-gray-900 text-white hover:bg-gray-800'}`}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="text-center mb-16">
             ...
             <p className="text-gray-500 max-w-3xl mx-auto text-lg leading-relaxed">
               Leading AI technology that understands your preferences and creates the perfect itinerary according to your wishes and budget.
             </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-12">
             <div className="bg-gray-50 rounded-3xl p-8 border border-gray-100 hover:shadow-xl hover:shadow-primary-100/50 transition-all duration-300 group text-center hover:-translate-y-2">
                ...
                <p className="text-gray-500 text-sm leading-relaxed">AI learns your preferences to suggest hidden gems you'll love.</p>
             </div>
             ...
             <div className="bg-gray-50 rounded-3xl p-8 border border-gray-100 hover:shadow-xl hover:shadow-primary-100/50 transition-all duration-300 group text-center hover:-translate-y-2">
                ...
                <p className="text-gray-500 text-sm leading-relaxed">Maximize your holiday with efficiently planned routes and schedules.</p>
             </div>
             ...
             <div className="bg-gray-50 rounded-3xl p-8 border border-gray-100 hover:shadow-xl hover:shadow-primary-100/50 transition-all duration-300 group text-center hover:-translate-y-2">
                ...
                <p className="text-gray-500 text-sm leading-relaxed">Visualize your journey with integrated maps and navigation.</p>
             </div>
             ...
             <div className="bg-gray-50 rounded-3xl p-8 border border-gray-100 hover:shadow-xl hover:shadow-primary-100/50 transition-all duration-300 group text-center hover:-translate-y-2">
                ...
                <p className="text-gray-500 text-sm leading-relaxed">Every itinerary is unique, tailored specifically to your travel style.</p>
             </div>
          </div>
      ...
      <div className="bg-gray-50 py-16 md:py-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
           <div className="flex justify-between items-end mb-12">
              <div>
                <h2 className="text-3xl md:text-5xl font-serif font-bold text-gray-900 mb-4">Curated Experiences</h2>
                <p className="text-gray-500 max-w-xl text-lg">Handpicked adventures that go beyond the ordinary.</p>
              </div>
              <Link to="/explore" className="hidden md:flex items-center text-primary-600 font-bold hover:text-primary-700 transition-colors">
                 View All Tours <ArrowRight className="w-4 h-4 ml-2" />
              </Link>
           </div>

           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {featuredProducts.map((product: { id: any; image: any; name: any; rating: any; location: any; description: any; currency: any; price: any; }) => (
                <div key={product.id} onClick={() => navigate(`/product/${product.id}`)} className="group cursor-pointer bg-white rounded-3xl overflow-hidden shadow-soft hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 border border-gray-100 flex flex-col h-full">
                   <div className="aspect-[4/3] relative overflow-hidden">
                      ...
                      <div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                          <span className="bg-white/90 backdrop-blur text-gray-900 px-6 py-3 rounded-full font-bold text-sm transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300">View Details</span>
                      </div>
                   </div>
                   <div className="p-8 flex-1 flex flex-col">
                      ...
                      <h3 className="font-serif font-bold text-2xl text-gray-900 mb-3 group-hover:text-primary-600 transition-colors line-clamp-2">{product.name}</h3>
                      <p className="text-gray-500 text-sm line-clamp-2 mb-6 leading-relaxed flex-1">{product.description}</p>
                      
                      <div className="pt-6 border-t border-gray-100 flex items-end justify-between">
                         <div>
                            <span className="text-xs text-gray-400 uppercase font-bold block mb-1">Starting from</span>
                            <span className="text-2xl font-bold text-gray-900">{product.currency} {product.price}</span>
                         </div>
           ...
           <div className="mt-12 text-center md:hidden">
              <Link to="/explore" className="inline-flex items-center px-6 py-3 bg-white border border-gray-200 rounded-xl font-bold text-gray-900 shadow-sm">
                 View All Tours <ArrowRight className="w-4 h-4 ml-2" />
              </Link>
           </div>
        </div>
      ...
      <div className="py-20 bg-gray-900 text-white relative overflow-hidden">
         ...
         <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <div className="text-center mb-16">
               <span className="text-primary-400 font-bold text-sm uppercase tracking-widest mb-3 block">Happy Travelers</span>
               <h2 className="text-3xl md:text-5xl font-serif font-bold text-white mb-6">Stories from the Road</h2>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
               {REVIEWS.map((review) => (
                  <div key={review.id} className="bg-gray-800 p-8 rounded-3xl border border-gray-700 relative hover:bg-gray-750 transition-colors">
                     <Quote className="w-10 h-10 text-primary-900 absolute top-6 right-6 opacity-50" />
                     <div className="flex items-center mb-6">
                        <img src={review.avatar} alt={review.user} className="w-12 h-12 rounded-full border-2 border-primary-500 mr-4" />
                        <div>
      ...
      <div className="bg-primary-600 py-24 px-4 text-center relative overflow-hidden">
         ...
         <div className="relative z-10 max-w-3xl mx-auto">
            <h2 className="text-4xl md:text-6xl font-serif font-bold text-white mb-8">Ready for your next adventure?</h2>
            <p className="text-primary-100 text-xl mb-10 max-w-2xl mx-auto">Join thousands of travelers who have found their perfect getaway with Trivgoo.</p>
            <div className="flex flex-col sm:flex-row justify-center gap-4">
               <Link to="/explore" className="px-10 py-4 bg-white text-primary-900 rounded-full font-bold text-lg hover:bg-gray-50 transition-all shadow-xl hover:shadow-2xl transform hover:-translate-y-1 active:scale-95">
                  Start Exploring
               </Link>
               <Link to="/login" className="px-10 py-4 bg-primary-700 text-white rounded-full font-bold text-lg border border-primary-500 hover:bg-primary-800 transition-all shadow-xl hover:shadow-2xl transform hover:-translate-y-1 active:scale-95">
                  Sign Up Now
               </Link>
            </div>
         </div>
